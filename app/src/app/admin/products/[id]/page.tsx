...
import { fetchProduct, updateProduct, deleteProductApi, fetchCategories } from '@/lib/api';
...
const BlockNoteEditor = dynamic(() => import('@/components/BlockNoteEditorWrapper'), {
	ssr: false,
	loading: () => (
		<div className='h-[400px] w-full bg-surface rounded-lg border border-border animate-pulse flex items-center justify-center text-text-muted'>
			載入編輯器中...
		</div>
	),
});
...
function toProductSpecs(value: unknown): ProductSpec[] {
...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
	...
	const handleSave = async (nextStatus: 'published' | 'draft') => {
		...
		if (purchaseLink.trim() && !normalizedPurchaseLink) {
			const msg = '前往購買連結格式不正確，請輸入完整網址或站內路徑（例如 /products）';
			setError(msg);
			showToast(msg, 'error');
			return;
		}
