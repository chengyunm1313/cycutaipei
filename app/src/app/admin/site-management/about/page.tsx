...
const ABOUT_PRESET_PAGES = [
	{ title: '品牌故事', slug: 'brand-story' },
	{ title: '歷史腳步', slug: 'history' },
	{ title: '工廠介紹', slug: 'factory' },
	{ title: '經營理念', slug: 'philosophy' },
	{ title: '品牌責任險', slug: 'brand-liability' },
] as const;

export default function AdminAboutManagePage() {
	...
	const loadPages = async () => {
		try {
			setLoading(true);
			const data = await fetchSiteContents({ type: 'about_page' });
			setPages(data);
		} catch (error) {
			console.error('載入關於我們管理頁失敗:', error);
	...
	const handleCreate = async () => {
		try {
			...
			const created = await createSiteContent({
				type: 'about_page',
				title: `關於我們頁面 ${nextIndex}`,
				slug: `about-${nextIndex}`,
				summary: '請填寫簡短說明',
				sortOrder: pages.length,
				isActive: 1,
				content: '<h1>這是大標題</h1><p>在此編輯訊息</p>',
				extraJson: JSON.stringify({ image2: '', image3: '' }),
			});
			setPages((prev) => [...prev, created]);
			setNotice('已新增一個關於我們頁面。');
			notifySiteContentUpdated();
		} catch (error) {
			console.error('新增關於我們頁面失敗:', error);
	...
	const handleCreatePresetPages = async () => {
		...
		const missingPresets = ABOUT_PRESET_PAGES.filter((preset) => {
		...
		try {
			...
			console.error('建立關於我們預設頁面失敗:', error);
			setNotice('建立預設頁面失敗，請稍後再試。');
		}
	};
	...
	const handleSave = async (item: ApiSiteContent) => {
		try {
			...
			console.error('儲存關於我們頁面失敗:', error);
	...
	const handleDelete = async (id: string) => {
		...
		try {
			...
			console.error('刪除關於我們頁面失敗:', error);
			setNotice('刪除失敗。');
		}
	};
	...
	return (
		<div className='max-w-6xl mx-auto space-y-6'>
			<div className='flex items-center justify-between gap-3'>
				<div>
					<h1 className='text-2xl lg:text-3xl font-extrabold text-text tracking-tight'>網站管理 - 關於我們 / 管理頁面</h1>
					<p className='text-text-light mt-2'>
						管理關於我們的頁面清單與排序，並前往自訂頁面編輯內容。
					</p>
				</div>
							...
							<tr key={item.id}>
								<td className='py-2.5 px-4'>
									<input
										...
										onChange={(event) => {
											...
											handlePatch(item.id, {
												title,
												slug: item.slug || slugifyAscii(title, 'about-page'),
											});
										}}
								...
								<td className='py-2.5 px-4'>
									<input
										...
										value={item.slug || ''}
										onChange={(event) =>
											handlePatch(item.id, { slug: slugifyAscii(event.target.value, 'about-page') })
										}
